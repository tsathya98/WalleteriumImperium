            ...
            mcp_response = {
                ...
                "mcp_format": {
                    ...
                    "extracted_data": {
                        ...
                        "metadata": {
                            "confidence_level": receipt_data.get("confidence", "medium"),
                ...
                "summary": {
                    ...
                    "store_name": receipt_data.get("store_info", {}).get("name", "Unknown"),
                    "transaction_date": receipt_data.get("store_info", {}).get("date", "Unknown")
                }
            }
    ...
    try:
        ...
        for receipt in receipt_history:
            ...
            for item in items:
                ...
                category_spending[category] = category_spending.get(category, 0) + item_total
                item_frequency[item_name] = item_frequency.get(item_name, 0) + 1
        
        # Generate insights
        most_frequent_store = max(store_spending, key=store_spending.get) if store_spending else "N/A"
        top_category = max(category_spending, key=category_spending.get) if category_spending else "N/A"
        most_bought_item = max(item_frequency, key=item_frequency.get) if item_frequency else "N/A"
        ...
        analysis_result = {
            ...
            "insights": {
                ...
                "average_per_receipt": round(total_spending / len(receipt_history), 2) if receipt_history else 0,
                "most_frequent_store": most_frequent_store,
                "top_spending_category": top_category,
                "most_purchased_item": most_bought_item
            },
            "breakdowns": {
                "by_store": dict(sorted(store_spending.items(), key=lambda x: x[1], reverse=True)),
                "by_category": dict(sorted(category_spending.items(), key=lambda x: x[1], reverse=True)),
                "item_frequency": dict(sorted(item_frequency.items(), key=lambda x: x[1], reverse=True)[:10])
            },
            "mcp_format": {
                "protocol_version": "1.0",
                "data_type": "receipt_analysis",
                ...
                "analysis_data": {
                    "spending_summary": {
                        ...
                        "average": round(total_spending / len(receipt_history), 2) if receipt_history else 0
                    },
                    "top_insights": {
